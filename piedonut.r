...
PieDonut=function(data,mapping,
  ...
  if(showRatioPie) {
    ...
    df$label=ifelse(df$ratio>=showRatioThreshold,
                    paste0(df$label,"\n(",scales::percent(df$ratio, accuracy = 0.1L),")"),
  ...
  if(!is.null(donuts)){
    ...
    df3
    if(ratioByGroup) {
      df3$ratio=scales::percent(df3$Freq/df3$group, accuracy = 0.1L)
    } else {
      df3$ratio<-scales::percent(df3$ratio1, accuracy = 0.1L)
    }
  ...
  if(showPieName & pieName != "") p1<-p1+annotate("text",x=0,y=0,label=pieName,size=titlesize,family=family)
  else if(showPieName) p1<-p1+annotate("text",x=0,y=0,label=pies,size=titlesize,family=family)
